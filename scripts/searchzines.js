    ...
    fetch("zinedata.json")
        ...
        .then(bands => {
            ...
            bands.forEach(band => {
                ...
                checkMatch(band.band_name, `Zine Name: ${band.band_name}`);
            ...
            if (matches.length === 0) {
                ...
                matches.forEach(({ band, matchText }) => {
                    ...
                    li.innerHTML = `<strong>Zine:</strong> ${band.band_name}`;
                    if (matchText) {
                        li.innerHTML += `<br><span style="color: #888; font-size: 14px;">${matchText}</span>`;
                    }

                    li.onclick = function() {
                        window.location.href = `zine.html?zine=${encodeURIComponent(band.band_name)}`;
                    };
        ...
        .catch(error => console.error("Error loading zine data:", error));
}
